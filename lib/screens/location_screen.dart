...
class LocationScreenState extends State<LocationScreen> {
  ...
  Future<void> getWDataByLocation() async {
    Location location = Location();
    await location.getCurrentLocation();
    var weatherData = await getWData(location.latitude, location.longitude);
    updateUI(weatherData);
  }

  Future<void> getWDataByCity(String cityName) async {
    var weatherData = await getCityWData(cityName);
    updateUI(weatherData);
  }

  Future<dynamic> getWData(double latitude, double longitude) async {
  ...
  Future<dynamic> getCityWData(String cityName) async {
    final String weatherUrl =
        "https://api.openweathermap.org/data/2.5/weather?q=$cityName&appid=4cc35b789e1d25c86e9a5aada44582bf&units=metric";
    NetworkHelper networkHelper = NetworkHelper(weatherUrl);
    return await networkHelper.getData();
  }
                  ...
                  children: <Widget>[
                    ...
                    GestureDetector(
                      onTap: () async {
                        ...
                        if (result != null) {
                          await getWDataByCity(result.toString()); // Fetch weather based on city name
                        }
                      },
                      child: Image.asset(
